...
Run examples:
    ...
    python scripts/sensitivity/post_std.py --run l1 --fixed-radius-step-for-l1 50
    python scripts/sensitivity/post_std.py --run r --fixed-l1-step-for-r 10
    python scripts/sensitivity/post_std.py --run trajectory --fixed-l1-step-for-trajectory 50 --fixed-radius-step-for-trajectory 50
"""
...
def evaluate_candidate_grid(
    ...
    mechanical_candidates = _scan_candidates(Data)
    records: list[dict] = []
    ...
    if not records:
        raise RuntimeError("No candidate could be evaluated for the selected grid.")

    df = pd.DataFrame(records).sort_values(["l1", "R"]).reset_index(drop=True)
    if verbose:
        print(f"  evaluated candidates: {len(df)}")
    return df
