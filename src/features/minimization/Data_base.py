...
class _MechanicalDataSet:
    ...
    Notes
    -----
    The current geometric formulation assumes a Type-1 well starting at the
    origin. Therefore ``P0`` is restricted to ``(0, 0)``.
    """
    ...
    def _validate_diameters(name: str, diameters: tuple[float, float]) -> None:
        ...
        if d_int >= d_ext:
            raise ValueError(
                f"The internal diameter for '{name}' must be smaller than the external diameter."
            )
...
class _BaseMesh:
    """Stores lithological intervals without mutating the user inputs."""
    ...
    def _normalize_intervals(intervals: list, name: str) -> list[tuple[float, float]]:
        ...
        for interval in intervals:
            ...
            if end <= start:
                raise ValueError(
                    f"Invalid interval in '{name}': end ({end}) must be greater than start ({start})."
                )
...
class DataSetTimeMixin:
    """Adds drilling-time optimization parameters to the mechanical dataset."""
    ...
    def _init_drilling_time_parameters(self, drilling_time_parameters: dict | None = None):
        ...
        self.buoyed_linear_weight_avg = float(
            np.mean(
                [
                    self.buoyed_linear_weight_command,
                    self.buoyed_linear_weight_heavy,
                    self.buoyed_linear_weight_drill,
                ]
            )
        )
...
class DataSet(_MechanicalDataSet, DataSetTimeMixin):
    def __init__(self, *args, drilling_time_parameters: dict | None = None, **kwargs):
        super().__init__(*args, **kwargs)
        self._init_drilling_time_parameters(drilling_time_parameters)

    def cache_signature(self) -> tuple:
        return super().cache_signature() + (tuple(sorted(self.drilling_time_parameters.items())),)


class mesh(_BaseMesh):
    """Geological mesh with one base ROP value for each lithology."""
    ...
    def __init__(
        ...
        for segment in self.segments:
            ...
            if rop is None and self.default_rop is None:
                raise ValueError(
                    f"Missing ROP for lithology '{lithology_name}'. Provide it in 'rop_values' or use 'default_rop'."
                )
            segment["rop"] = float(self.default_rop if rop is None else rop)

        self._starts = [segment["start"] for segment in self.segments]
        self._ends = [segment["end"] for segment in self.segments]
    ...
    def _check_overlaps(self) -> None:
        ...
        for previous, current in zip(ordered[:-1], ordered[1:]):
            if current["start"] < previous["end"]:
                raise ValueError(
                    "The geological intervals overlap. Please provide a non-overlapping depth partition."
                )
    ...
    def segment_at_depth(self, depth: float) -> dict:
        ...
        if self.default_rop is None:
            raise ValueError(
                f"Depth {d:.3f} m is outside the geological mesh and no default ROP was provided."
            )
    ...
    def cache_signature(self) -> tuple:
        seg_tuple = tuple(
            (seg["lithology"], seg["start"], seg["end"], seg.get("rop")) for seg in self.segments
        )
